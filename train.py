...
class KWSDataModule(LightningDataModule):
    def __init__(self, path, batch_size=128, num_workers=2, n_fft=512,
                 n_mels=128, win_length=None, hop_length=256, class_dict={}, patch_num=16,
                 **kwargs):
...
# Args and Plots (KWS & Transformer)
...
if __name__ == "__main__":
    ...
    model = KWSModel(num_classes=args.num_classes, epochs=args.max_epochs, lr=args.lr, patch_dim=patch_dim,
                     seqlen=seq_len)
    ...
    model = model.load_from_checkpoint(os.path.join(args.path, "checkpoints", "kws-best-acc.ckpt"))
    model.eval()
    script = model.to_torchscript()

    torch.jit.save(script, "kws-best-acc.pt")